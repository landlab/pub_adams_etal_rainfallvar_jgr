...
Tr = csvread('_/data/lowRvar/nonsteady/20mGeneral/durations.txt');
P = csvread('_/data/lowRvar/nonsteady/20mGeneral/intensities.txt');
%d = csvread('_/data/lowRvar/nonsteady/20mGeneral/depths.txt');
%disp(storm_array);
...
sall = sall(1:401);
%disp(mean(sall))
Er = csvread('_/data/lowRvar/nonsteady/20mGeneral/outlet_median_erosion_rates_tc0.txt');
Er = Er(:,1);
%Er(find(Er < 0)) = 0.;
...
%disp(Er);
A=aall(ids); %aall = storm duration
S=sall(ids); %sall = storm intensity
I=uall(ids); %uall = median erosion rate
fig=1;

log10A=log10(A);  %aall = storm duration
log10S=log10(S);  %sall = storm intensity
log10I=log10(I);  %uall = median erosion rate
